...
from .clients import get_client, validate_client_network
...
def download_snapshot(
    ...
    Returns:
        Path to extracted snapshot directory
    """
    validate_client_network(client_name, network)

    url, compression = get_snapshot_url(client_name, network, block)
    output_dir = Path(data_dir) / "snapshots" / client_name
    ...
    # Full pipeline: download | decompress | extract
    full_cmd = f"{download_cmd} | tar -I {decompress} -xC '{output_dir}'"

    run(["sh", "-c", full_cmd])

    console.print(f"[green]Snapshot downloaded to {output_dir}[/green]")
    return output_dir
...
def get_snapshot_path(client_name: str, data_dir: str) -> Path:
    """Get the expected snapshot path for a client."""
    return Path(data_dir) / "snapshots" / client_name
